...
$firstDay = $wholeYear ? EarthianDate::firstDayOfYear($year)
  : EarthianDate::firstDayOfMonth($year, $month);
$lastDay = $wholeYear ? EarthianDate::lastDayOfYear($year)
  : EarthianDate::lastDayOfMonth($year, $month);
...
while (!$done) {
  for ($dayOfWeek = 1; $dayOfWeek <= 7; $dayOfWeek++) {
    ...
    $class = ($earthDate->month % 2 ? 'odd' : 'even') . 'MonthWeek' .
      ($dayOfWeek == 1 || $dayOfWeek == 7 ? 'end' : 'day');
    println("<td class='$class'>");
    println("<div id='$id' class='dayNum'>" .
