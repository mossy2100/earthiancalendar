...
while (!feof($fp))
{
	...
	if (($earthDate->month == 1 && $earthDate->isLaterThan($first)) || ($earthDate->month == 12 && $earthDate->isEarlierThan($last))) {
		print("</font>");
	}
	...
	if ($earthDate->month == 12 && $earthDate->isEarlierThanOrEqualTo($last) && $time < $earliestTime) {
