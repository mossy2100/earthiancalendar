...
class EarthianDate
{
	 ...
	 * @var string[]
	 */
	public static $dayNames = array(1 => 'Sunday', 'Lunaday', 'Marsday', 'Mercuryday', 'Jupiterday', 'Venusday', 'Saturnday');
	 ...
	 * @var string[]
	 */
	public static $monthNames = array(1 => 'Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpius', 'Sagittarius', 'Capricornus', 'Aquarius', 'Pisces');
	 ...
	 * @var string[]
	 */
	public static $abbrevMonthNames = array(1 => 'Ari', 'Tau', 'Gem', 'Cnc', 'Leo', 'Vir', 'Lib', 'Sco', 'Sgr', 'Cap', 'Aqr', 'Psc');
	...
	function __construct()
	{
		...
		elseif ($nArgs == 1)
		{
			...
			elseif (is_string($args[0]) && preg_match("/^\d{4}\\".self::sep."\d{2}\\".self::sep."\d{2}$/", $args[0]))
			{
	...
	public static function dateSelector($id, $selectedValue, $attribs, $minYear, $maxYear)
	{
		...
		$yearSelector = self::yearSelector($id."_year", $selectedValue->year, $attribs, $minYear, $maxYear);
	...
	public static function firstDayOfWeek($mixed, $week = null)
	{
		...
		else
		{
			...
			elseif (is_array($mixed) && is_numeric($mixed['year']) && is_numeric($mixed['week']) && $week == null)
			{
			...
			$firstDayOfWeek = self::addDays($firstDayOfYear, 1 - $dayOfWeek + (($week - $weekOfYear) * 7));
	...
	public function weekOfYear()
	{
		...
		return array('year' => $year, 'week' => $weekOfYear);
	}
}
?>