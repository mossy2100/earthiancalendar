...
// =================
//
// Date/Time Library - Copernicus Calendar extension - JavaScript version
...
function EarthianDate()
{
	...
	else if (nArgs == 1)
	{
		...
		{
			// this pattern matches -YY.MM.DD:
			var rx = "/^\-?\d+\\" + EarthianDate.sep + "\d+\\" + EarthianDate.sep + "\d+$/ig";
 ...
 * @var string[]
 */
EarthianDate.dayNames = [undefined, 'Sunday', 'Lunaday', 'Marsday', 'Mercuryday', 'Jupiterday', 'Venusday', 'Saturnday'];
 ...
 * @var string[]
 */
EarthianDate.monthNames = [undefined, 'Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpius', 'Sagittarius', 'Capricornus', 'Aquarius', 'Pisces'];


/**
 * The Modified Julian Day number of day 0 of the Copernicus Calendar,
 * i.e. -1,12,30, the day before the first day of the calendar.
 */
EarthianDate.mjdDay0 = 54179;
...
EarthianDate.dateSelector = function(id, selectedValue, attribs, minYear, maxYear)
{
	...
	var yearSelector = EarthianDate.yearSelector(id + "_year", selectedValue.year, attribs, minYear, maxYear);
...
{
	return fourDigits(this.year) + EarthianDate.sep + twoDigits(this.month) + EarthianDate.sep + twoDigits(this.day);
};
...
EarthianDate.prototype.weekOfYear = function()
{
	...
	var weekOfYear = intdiv(dayOfYear - dayOfWeek + 3, 7) + 1;
	var year = this.year;
	if (weekOfYear == 0)
	{
		year--;
