...
function dtlSelector($name, $dts, $format, $onchange, $minYear, $maxYear, $increment) {
  ...
  if ($date) {
    ...
    $monthFound = false;
    $dayFound = false;
    $monthFormats = ['Month', 'month', 'MONTH', 'Mon', 'mon', 'MON', 'MM', 'M'];
    $dayFormats = ['Dth', 'DD', 'D'];
  }
  ...
  while ($pos < $len) {
      ...
      if (!$yearFound && $ch == 'Y') {
        ...
        if (!$monthFound && ($ch == 'M' || $ch == 'm')) {
          ...
          if (!$dayFound && $ch == 'D') {
            ...
            if ($dayFound) {
              dtlDayOfMonthSelector($name . '_day', $day, $nDays, $dayFormat,
                "$updateFunction('$name')");
    ...
    if ($time) {
      if (!$hoursFound && $ch == 'H') {
        ...
        if (!$minutesFound && $ch == 'm') {
          if (substr($format, $pos, 2) == 'mm') {
            $minutesFound = true;
            dtlMinutesSelector($name . '_minutes', $minutes, $minutesIncrement,
              "$updateFunction('$name')");
            $pos += 2;
            $replacement = true;
          }
          ...
          if (!$secondsFound && $ch == 's') {
            if (substr($format, $pos, 2) == 'ss') {
              $secondsFound = true;
              dtlSecondsSelector($name . '_seconds', $seconds, $secondsIncrement,
                "$updateFunction('$name')");
              $pos += 2;
              $replacement = true;
            }
...
function dtlDateSelector($name, $ds = "0000-00-00", $format = 'DMonYYYY', $onchange = '',
...
function dtlTimeSelector($name, $ts = "00:00:00", $format = 'HH:mm', $onchange = '',
  $increment = 5) {
...
function dtlDateTimeSelector($name, $dts = "0000-00-00 00:00:00", $format = 'DMonYYYY HH:mm',
  $onchange = '', $minYear = 2001, $maxYear = 2010, $increment = 5) {
  if ($dts == '') {
    $dts = '0000-00-00 00:00:00';
  }
  if ($format == '') {
    $format = 'DMonYYYY HH:mm';
  }
