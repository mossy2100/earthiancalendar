...
function dtlIsInt(n)
{
	...
	if (typeof(n) == 'number' && n == Math.round(n))
		return true;
	else if (typeof(n) == 'string' && n == dtlToInt(n))
		return true;
	else
		return false;
}
...
function dtlIsDateTimeStr(dts)
{
	...
	return typeof(dts) == 'string' && dts.length == 19 && (dts.match(/\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/)).length == 1;
}
...
function dtlDaysInMonth(year, month)
{
	...
	if (!dtlIsInRange(month, 1, 12))
		return false;
	if (dtlInArray(month, new Array(1, 3, 5, 7, 8, 10, 12)))
		return 31;
	else if (dtlInArray(month, new Array(4, 6, 9, 11)))
		return 30;
	else // month == 2
...
function dtlIsValidDate(ds)
{
	...
	// check limits:
	if (!dtlIsInRange(da['year'], 0, 9999) || !dtlIsInRange(da['month'], 1, 12) || !dtlIsInRange(da['day'], 1, 31))
		return false;
...
function dtlIsValidTime(ts)
{
	...
	return dtlIsInRange(ta['hours'], 0, 23) && dtlIsInRange(ta['minutes'], 0, 59) && dtlIsInRange(ta['seconds'], 0, 59);
}
...
function dtlLocalDateTime()
{
	...
	var dNow = new Date();
	return dtlDateTimeStr(dNow.getFullYear(), dNow.getMonth() + 1, dNow.getDate(), dNow.getHours(), dNow.getMinutes(), dNow.getSeconds());
}
...
var dtlLanguages = {};
dtlLanguages.EN = {};
dtlLanguages.EN.monthNames = [undefined, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
dtlLanguages.EN.dayNames = [undefined, 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
...
function dtlJSDToDateStr(jsd)
{
	...
	return dtlZeroPad(jsd.getFullYear(), 4) + '-' + dtlZeroPad(jsd.getMonth() + 1) + '-' + dtlZeroPad(jsd.getDate());
}
...
function dtlDateTimeStrToJSD(ds)
{
	...
	return new Date(da['year'], da['month'] - 1, da['day'], da['hours'], da['minutes'], da['seconds']);
	return jsd;
}
...
function dtlUpdateTimeSelectors(name, hours, minutes, seconds)
{
	...
	{
		increment = (dtlToInt(selMinutes.options[1].value) - dtlToInt(selMinutes.options[0].value)) * 60;
	}
	else
	{
		increment = 1;
