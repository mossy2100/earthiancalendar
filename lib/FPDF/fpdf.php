...
class FPDF {
	...
	function FPDF($orientation = 'P', $unit = 'mm', $format = 'A4') {
		...
		//Standard fonts
		$this->CoreFonts =
			['courier'    => 'Courier', 'courierB' => 'Courier-Bold', 'courierI' => 'Courier-Oblique',
	...
	function Rect($x, $y, $w, $h, $style = '') {
		...
		$this->_out(sprintf('%.2F %.2F %.2F %.2F re %s', $x * $this->k, ($this->h - $y) * $this->k,
	...
	function SetFont($family, $style = '', $size = 0) {
		...
		if (!isset($this->fonts[$fontkey])) {
			...
			if (isset($this->CoreFonts[$fontkey])) {
				...
				$this->fonts[$fontkey] =
					['i' => $i, 'type' => 'core', 'name' => $name, 'up' => -100, 'ut' => 50, 'cw' => $cw];
			}
			else {
	...
	function Cell($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = false,
		...
		if ($txt !== '') {
			...
			if ($link) {
				$this->Link($this->x + $dx, $this->y + .5 * $h - .5 * $this->FontSize,
	...
	function Image($file, $x = null, $y = null, $w = 0, $h = 0, $type = '', $link = '') {
		...
		$this->_out(sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /I%d Do Q', $w * $this->k, $h * $this->k,
			$x * $this->k, ($this->h - ($y + $h)) * $this->k, $info['i']));
		if ($link) {
	...
	function _parsejpg($file) {
		...
		fclose($f);
		return ['w'    => $a[0], 'h' => $a[1], 'cs' => $colspace, 'bpc' => $bpc, 'f' => 'DCTDecode',
						'data' => $data];
	}
	...
	function _parsepng($file) {
		...
		$this->_readstream($f, 4);
		$parms =
			'/DecodeParms <</Predictor 15 /Colors ' . ($ct == 2 ? 3 : 1) . ' /BitsPerComponent ' . $bpc .
			' /Columns ' . $w . '>>';
		...
		fclose($f);
		return ['w'     => $w, 'h' => $h, 'cs' => $colspace, 'bpc' => $bpc, 'f' => 'FlateDecode',
	...
	function _putpages() {
		...
		for ($n = 1; $n <= $nb; $n++) {
			...
			if (isset($this->PageSizes[$n])) {
				$this->_out(sprintf('/MediaBox [0 0 %.2F %.2F]', $this->PageSizes[$n][0],
					$this->PageSizes[$n][1]));
			}
