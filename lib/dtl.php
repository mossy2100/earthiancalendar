...
function dtlDateStr($year = 0, $month = 0, $day = 0) {
  ...
  if (!dtlIsInt($year) || $year < 0 || $year > 9999 || !dtlIsInt($month) || $month < 0 ||
    $month > 99 || !dtlIsInt($day) || $day < 0 || $day > 99) {
    return false;
  }
...
function dtlDateTimeStr($multi = '0000-00-00', $multi2 = '00:00:00', $day = 0, $hours = 0,
  $minutes = 0, $seconds = 0) {
...
function dtlUTSToDateTimeStr($unixTimestamp) {
  ...
  return dtlDateTimeStr($dta['year'], $dta['mon'], $dta['mday'], $dta['hours'], $dta['minutes'],
    $dta['seconds']);
}
...
function dtlDateTimeArrayToStr($dta) {
  ...
  // * returns false for invalid input
  if (!is_array($dta) || !isset($dta['year']) || !isset($dta['month']) || !isset($dta['$day']) ||
    !isset($dta['hours']) || !isset($dta['minutes']) || !isset($dta['$seconds'])) {
    return false;
  }
  return dtlDateTimeStr($dta['year'], $dta['month'], $dta['day'], $dta['hours'], $dta['minutes'],
    $dta['seconds']);
}
...
function dtlProcessDateParams($multi, &$year, &$month, &$day) {
  ...
  if ($da = dtlDateStrToArray($multi)) // returns false if $multi not a DateStr
  {
    ...
    if (dtlIsInt($multi) && dtlIsInt($month) && dtlIsInt($day)) {
      ...
      if (is_array($multi) && count($multi) == 3 &&
...
function dtlProcessDateTimeParams($multi, &$year, &$month, &$day, &$hours, &$minutes, &$seconds) {
  ...
  if ($dta = dtlDateTimeStrToArray($multi)) // returns false if $multi not a DateTimeStr
  {
    ...
    if (dtlIsInt($multi) && dtlIsInt($month) && dtlIsInt($day) && dtlIsInt($hours) &&
      dtlIsInt($minutes) && dtlIsInt($seconds)) {
      // syntax 2: multi is the year
      $year = $multi;
      return true;
    }
    else {
      if (is_array($multi) && count($multi) == 6 &&
        dtlIsInt($multi['year']) && dtlIsInt($multi['month']) && dtlIsInt($multi['day']) &&
        dtlIsInt($multi['hours']) && dtlIsInt($multi['minutes']) && dtlIsInt($multi['seconds'])) {
        extract($multi); // sets $year = $multi['year'], etc.
        return true;
...
function dtlIsValidDateTime($multi, $month = 0, $day = 0, $hours = 0, $minutes = 0, $seconds = 0,
  $proleptic = false) {
...
function dtlMakeDateTimeValid(&$year, &$month, &$day, &$hours, &$minutes, &$seconds,
  $proleptic = false) {
  ...
  // check param types:
  if (!dtlIsInt($year) || !dtlIsInt($month) || !dtlIsInt($day) || !dtlIsInt($hours) ||
    !dtlIsInt($minutes) || !dtlIsInt($seconds)) {
    return false;
  }
...
function dtlAddLanguage($languageCode, $monthNames, $dayNames, $abbrevDayNames = 0) {
  ...
  // check $monthNames
  if (!is_array($monthNames) || count($monthNames) != 12 || !isset($monthNames[1]) ||
    !isset($monthNames[2]) || !isset($monthNames[3]) || !isset($monthNames[4]) ||
    !isset($monthNames[5]) || !isset($monthNames[6]) || !isset($monthNames[7]) ||
    !isset($monthNames[8]) || !isset($monthNames[9]) || !isset($monthNames[10]) ||
    !isset($monthNames[11]) || !isset($monthNames[12])) {
    return false;
  }
  // check $dayNames
  if (!is_array($dayNames) || count($dayNames) != 7 || !isset($dayNames[1]) ||
    !isset($dayNames[2]) || !isset($dayNames[3]) || !isset($dayNames[4]) || !isset($dayNames[5]) ||
    !isset($dayNames[6]) || !isset($dayNames[7])) {
    return false;
  }
  // check $abbrevDayNames
  if ((!dtlIsInt($abbrevDayNames) || $abbrevDayNames < 0) &&
    (!is_array($abbrevDayNames) || count($abbrevDayNames) != 7 || !isset($abbrevDayNames[1]) ||
      !isset($abbrevDayNames[2]) || !isset($abbrevDayNames[3]) || !isset($abbrevDayNames[4]) ||
      !isset($abbrevDayNames[5]) || !isset($abbrevDayNames[6]) || !isset($abbrevDayNames[7]))) {
    return false;
  }
                      ...
                      if ($ch == 's') {
                        ...
                        if ($ch == 'x') {
                          if (substr($format, $dtlFormatCounter, 2) == 'xm') {
                            ...
                            if (substr($format, $dtlFormatCounter, 4) == 'x.m.') {
                              $hours = dtlGetHours($dts);
                              if ($hours !== false) {
                                dtl24HourTo12($hours, $xm);
...
function dtlHadLeapSecond($ds) {
  ...
  return in_array($ds, [
    '1971-12-31', '1972-06-30', '1972-12-31', '1973-12-31', '1974-12-31', '1975-12-31',
    '1976-12-31', '1977-12-31', '1978-12-31', '1979-12-31', '1981-06-30', '1982-06-30',
    '1983-06-30', '1985-06-30', '1987-12-31', '1989-12-31', '1990-12-31', '1992-06-30',
    '1993-06-30', '1994-06-30', '1995-12-31', '1997-06-30', '1998-12-31']);
}
...
function dtlEaster($year = 0) {
  ...
  $i = $i -
    dtlIntDiv($i, 28) * (1 - dtlIntDiv($i, 28) * dtlIntDiv(29, $i + 1) * dtlIntDiv(21 - $n, 11));
...
/**
 * This reverses the fields in a date string, i.e. converts a DD-MM-YYYY date to YYYY-MM-DD or
 * vice-versa.
 *
 * @param string $date
 */
