...
function dtlDateStr($year = 0, $month = 0, $day = 0)
{
	...
	if (!dtlIsInt($year) || $year < 0 || $year > 9999 || !dtlIsInt($month) || $month < 0 || $month > 99 || !dtlIsInt($day) || $day < 0 || $day > 99)
	return false;
...
function dtlTimeStr($hours = 0, $minutes = 0, $seconds = 0)
{
	...
	if (!dtlIsInRange($hours, 0, 99) || !dtlIsInRange($minutes, 0, 99) || !dtlIsInRange($seconds, 0, 99))
	return false;
	return dtlZeroPad($hours).':'.dtlZeroPad($minutes).':'.dtlZeroPad($seconds);
}


function dtlDateTimeStr($multi = '0000-00-00', $multi2 = '00:00:00', $day = 0, $hours = 0, $minutes = 0, $seconds = 0)
{
...
function dtlIsDateStr($ds)
{
	...
	return is_string($ds) && strlen($ds) == 10 && preg_match('/\d\d\d\d-\d\d-\d\d/', $ds) == 1;
}
...
function dtlIsDateTimeStr($dts)
{
	...
	return is_string($dts) && strlen($dts) == 19 && preg_match('/\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/', $dts) == 1;
}
...
function dtlUTSToDateTimeStr($unixTimestamp)
{
	...
	if (!$dta)
	return false;
	return dtlDateTimeStr($dta['year'], $dta['mon'], $dta['mday'], $dta['hours'], $dta['minutes'], $dta['seconds']);
}
...
function dtlDateArrayToStr($da)
{
	...
	// * returns false for invalid input
	if (!is_array($da) || !isset($da['year']) || !isset($da['month']) || !isset($da['$day']))
	return false;
...
function dtlTimeArrayToStr($ta)
{
	...
	// * returns false for invalid input
	if (!is_array($ta) || !isset($ta['hours']) || !isset($ta['minutes']) || !isset($ta['$seconds']))
	return false;
...
function dtlDateTimeArrayToStr($dta)
{
	...
	// * returns false for invalid input
	if (!is_array($dta) || !isset($dta['year']) || !isset($dta['month']) || !isset($dta['$day']) || !isset($dta['hours']) || !isset($dta['minutes']) || !isset($dta['$seconds']))
	return false;
	return dtlDateTimeStr($dta['year'], $dta['month'], $dta['day'], $dta['hours'], $dta['minutes'], $dta['seconds']);
}
...
function dtlDateTimeStrToArray($dts)
{
	...
	return false;
	$dtParts = explode(' ', $dts);
	return array_merge(dtlDateStrToArray($dtParts[0]), dtlTimeStrToArray($dtParts[1]));
}
...
function dtlDateTimeToMJD($dts)
{
	...
	$frac = ($dta['hours'] / 24) + ($dta['minutes'] / 1440) + ($dta['seconds'] / 86400);
	$mjd += $frac;
	return $mjd;
}
...
function dtlProcessDateParams($multi, &$year, &$month, &$day)
{
	...
	else if (dtlIsInt($multi) && dtlIsInt($month) && dtlIsInt($day))
	{
		// syntax 2: multi is the year
		$year = $multi;
		return true;
	}
	else if (is_array($multi) && count($multi) == 3 &&
	dtlIsInt($multi['year']) && dtlIsInt($multi['month']) && dtlIsInt($multi['day']))
	{
...
function dtlProcessTimeParams($multi, &$hours, &$minutes, &$seconds)
{
	...
	else if (dtlIsInt($multi) && dtlIsInt($minutes) && dtlIsInt($seconds))
	{
	...
	dtlIsInt($multi['hours']) && dtlIsInt($multi['minutes']) && dtlIsInt($multi['seconds']))
	{
...
function dtlProcessDateTimeParams($multi, &$year, &$month, &$day, &$hours, &$minutes, &$seconds)
{
	...
	else if (dtlIsInt($multi) && dtlIsInt($month) && dtlIsInt($day) && dtlIsInt($hours) && dtlIsInt($minutes) && dtlIsInt($seconds))
	{
		// syntax 2: multi is the year
		$year = $multi;
		return true;
	}
	else if (is_array($multi) && count($multi) == 6 &&
	dtlIsInt($multi['year']) && dtlIsInt($multi['month']) && dtlIsInt($multi['day']) &&
	dtlIsInt($multi['hours']) && dtlIsInt($multi['minutes']) && dtlIsInt($multi['seconds']))
	{
		extract($multi); // sets $year = $multi['year'], etc.
...
function dtlIsValidTime($multi, $minutes = 0, $seconds = 0, $duration = false, $allowLeapSeconds = false)
{
	...
	return false;
	return dtlIsInRange($minutes, 0, 59) && dtlIsInRange($seconds, 0, $allowLeapSeconds ? 60 : 59);
}


function dtlIsValidDateTime($multi, $month = 0, $day = 0, $hours = 0, $minutes = 0, $seconds = 0, $proleptic = false)
{
	...
	// * $multi is therefore a multi-purpose parameter
	if (!dtlProcessDateTimeParams($multi, $year, $month, $day, $hours, $minutes, $seconds))
	return false;
...
function dtlMakeTimeValid(&$hours, &$minutes, &$seconds, $duration = false)
{
	...
	$valid = dtlIsInRange($hours2, 0, 23) && dtlIsInRange($minutes2, 0, 59) && dtlIsInRange($seconds2, 0, 59);
	if ($valid)
	{
		$hours = $hours2;
		$minutes = $minutes2;
...
function dtlMakeDateTimeValid(&$year, &$month, &$day, &$hours, &$minutes, &$seconds, $proleptic = false)
{
	...
	// check param types:
	if (!dtlIsInt($year) || !dtlIsInt($month) || !dtlIsInt($day) || !dtlIsInt($hours) || !dtlIsInt($minutes) || !dtlIsInt($seconds))
	return false;

	// do we need to do anything?
	if (dtlIsValidDateTime($year, $month, $day, $hours, $minutes, $seconds, $proleptic))
	return true;
...
function dtlGetHours($dts = '')
{
	...
	if ($dts == '')
	$dts = dtlNow();
	else if (!dtlIsTimeStr($dts) && !dtlIsDateTimeStr($dts))
	return false;
	$ts = dtlGetTime($dts);
...
function dtlSetQuarter(&$dts, $quarter)
{
	...
	// check for valid input:
	if (!dtlIsInt($quarter) || !dtlIsValidDate($dts) && !dtlIsValidDateTime($dts))
	return false;
	// do we need to do anything?
...
function dtlSetHours(&$dts, $hours)
{
	...
	$type = 'time';
	else if (dtlIsValidDateTime($dts))
	$type = 'datetime';
	else
	return false;
	// do we need to do anything?
...
function dtlAdd($dts, $n, $units = 'days')
{
	...
	else if ($units == 'quarters' || $units == 'quarter')
...
function dtlAddYears($dts, $nYears)
{
	...
	if (!dtlIsInt($nYears) || (!dtlIsValidDate($dts) && !dtlIsValidDateTime($dts)))
	return false;
...
function dtlAddQuarters($dts, $nQuarters)
{
	...
	//   or if result is not a valid date/datetime
	// * returns true if result is a valid date/datetime
	if (!dtlIsInt($nQuarters) || (!dtlIsValidDate($dts) && !dtlIsValidDateTime($dts)))
	return false;
...
function dtlAddMonths($dts, $nMonths)
{
	...
	//   or if result is not a valid date/datetime
	// * returns true if result is a valid date/datetime
	if (!dtlIsInt($nMonths) || (!dtlIsValidDate($dts) && !dtlIsValidDateTime($dts)))
	return false;
...
function dtlAddHours($dts, $nHours)
{
	...
	if (!dtlIsInt($nHours) || (!dtlIsValidTime($dts) && !dtlIsValidDateTime($dts)))
	return false;
...
function dtlAddMinutes($dts, $nMinutes)
{
	...
	//   or if result is not a valid time/datetime
	// * returns true if result is a valid time/datetime
	if (!dtlIsInt($nMinutes) || (!dtlIsValidTime($dts) && !dtlIsValidDateTime($dts)))
	return false;
...
function dtlAddSeconds(&$dts, $nSeconds)
{
	...
	if (!dtlIsInt($nSeconds) || (!dtlIsValidTime($dts) && !dtlIsValidDateTime($dts)))
	return false;
...
function dtlIsEarlierThan($dts1, $dts2)
{
	...
	(dtlIsValidTime($dts1) && dtlIsValidTime($dts2)) ||
...
// English:
$dtlLanguages['EN'] = array();
$dtlLanguages['EN']['monthNames'] = array(1 => 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');
$dtlLanguages['EN']['dayNames'] = array(1 => 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday');
...
function dtlAddLanguage($languageCode, $monthNames, $dayNames, $abbrevDayNames = 0)
{
	...
	// check $monthNames
	if (!is_array($monthNames) || count($monthNames) != 12 || !isset($monthNames[1]) || !isset($monthNames[2]) || !isset($monthNames[3]) || !isset($monthNames[4]) || !isset($monthNames[5]) || !isset($monthNames[6]) || !isset($monthNames[7]) || !isset($monthNames[8]) || !isset($monthNames[9]) || !isset($monthNames[10]) || !isset($monthNames[11]) || !isset($monthNames[12]))
	return false;
	// check $dayNames
	if (!is_array($dayNames) || count($dayNames) != 7 || !isset($dayNames[1]) || !isset($dayNames[2]) || !isset($dayNames[3]) || !isset($dayNames[4]) || !isset($dayNames[5]) || !isset($dayNames[6]) || !isset($dayNames[7]))
	return false;
	// check $abbrevDayNames
	if ((!dtlIsInt($abbrevDayNames) || $abbrevDayNames < 0) && (!is_array($abbrevDayNames) || count($abbrevDayNames) != 7 || !isset($abbrevDayNames[1]) || !isset($abbrevDayNames[2]) || !isset($abbrevDayNames[3]) || !isset($abbrevDayNames[4]) || !isset($abbrevDayNames[5]) || !isset($abbrevDayNames[6]) || !isset($abbrevDayNames[7])))
	return false;
	// parameters look ok, let's include them in the languages array:
	global $dtlLanguages;
	dtlNormalizeLanguageCode($languageCode);
	$dtlLanguages[$languageCode] = array();
...
function dtlAbbrevMonthName($month, $languageCode = '', $nChars = 3)
{
	...
	if (!dtlIsInt($month) || $month < 0 || $month > 12 || !dtlIsInt($nChars) || $nChars <= 0)
	return false;
	dtlNormalizeLanguageCode($languageCode);
	if (!dtlIsSupportedLanguage($languageCode))
	return false;
	...
	else
	{
		...
		else if (is_array($abbrevMonthNames))
...
function dtlFormat($dts, $format = '', $mustBeValid = true)
{
	...
	while ($dtlFormatCounter < $len)
	{
		...
		if ($ch == 'Y')
		{
			...
			else if (substr($format, $dtlFormatCounter, 2) == 'YY')
			{
				$year = dtlGetYear($dts);
				if ($year !== false)
		...
		else if ($ch == 'M')
		{
			...
			else if (substr($format, $dtlFormatCounter, 3) == 'MON')
			{
			...
			else if (substr($format, $dtlFormatCounter, 5) == 'Month')
			dtlReplace('Month', dtlGetMonthName($dts));
			else if (substr($format, $dtlFormatCounter, 3) == 'Mon')
			dtlReplace('Mon', dtlGetAbbrevMonthName($dts));
			else if (substr($format, $dtlFormatCounter, 2) == 'MM')
			{
		...
		else if ($ch == 'D')
		{
			...
			else if (substr($format, $dtlFormatCounter, 3) == 'DAY')
			{
			...
			else if (substr($format, $dtlFormatCounter, 9) == 'Dayofweek')
			dtlReplace('Dayofweek', dtlGetDayName($dts));
			else if (substr($format, $dtlFormatCounter, 3) == 'Day')
			dtlReplace('Day', dtlGetAbbrevDayName($dts));
			else if (substr($format, $dtlFormatCounter, 3) == 'DOY')
			dtlReplace('DOY', dtlGetDayOfYear($dts));
			else if (substr($format, $dtlFormatCounter, 3) == 'Dth')
			{
				$day = dtlGetDay($dts);
				if ($day !== false)
			...
			else if (substr($format, $dtlFormatCounter, 2) == 'DD')
			{
				$day = dtlGetDay($dts);
				if ($day !== false)
				$day = dtlZeroPad($day);
		...
		else if ($ch == 'x')
		{
			...
			else if (substr($format, $dtlFormatCounter, 4) == 'x.m.')
			{
				$hours = dtlGetHours($dts);
				if ($hours !== false)
				{
...
function dtlDaysInMonths($year, $month, $proleptic = false)
{
	...
	{
		case 1: return 31;
		case 2: return 59 + $leapDay;
		case 3: return 90 + $leapDay;
		case 4: return 120 + $leapDay;
		case 5: return 151 + $leapDay;
		case 6: return 181 + $leapDay;
		case 7: return 212 + $leapDay;
		case 8: return 243 + $leapDay;
		case 9: return 273 + $leapDay;
		case 10: return 304 + $leapDay;
		case 11: return 334 + $leapDay;
		case 12: return 365 + $leapDay;
	}
}
...
function dtlHadLeapSecond($ds)
{
	...
	return in_array($ds, array(
	'1971-12-31', '1972-06-30', '1972-12-31', '1973-12-31', '1974-12-31', '1975-12-31', '1976-12-31', '1977-12-31', '1978-12-31', '1979-12-31', '1981-06-30', '1982-06-30', '1983-06-30', '1985-06-30', '1987-12-31', '1989-12-31', '1990-12-31', '1992-06-30', '1993-06-30', '1994-06-30', '1995-12-31', '1997-06-30', '1998-12-31'));
}
...
function dtlFirstDayOfYear($mixed = '')
{
	...
	else if (dtlIsValidDate($mixed) || dtlIsValidDateTime($mixed))
	{
...
function dtlFirstDayOfWeek($mult = '', $woy = 0)
{
	...
	else if (dtlIsInRange($mult, 0, 9999) && dtlIsInRange($woy, 0, 53))
	{
...
function dtl12HourTo24(&$hours, $xm)
{
	...
	$xm = strtoupper($xm);
	if (!dtlIsInt($hours) || $hours < 1 || $hours > 12 || ($xm != 'AM' && $xm != 'PM'))
	return false;
	if ($hours == 12 && $xm == 'AM')
	$hours = 0;
	else if ($hours < 12 && $xm == 'PM')
	$hours += 12;
}
...
/**
 * This reverses the fields in a date string, i.e. converts a DD-MM-YYYY date to YYYY-MM-DD or vice-versa.
 *
 * @param string $date
 */
