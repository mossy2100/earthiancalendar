...
// Adds zeroes to fill decimal places, and adds commas to show thousands, millions, etc.
{
  ...
  if (decimals > 0) {
    ...
    var strRight = padLeft(right, decimals, '0');
...
function formatNumberField(fieldName, decimalPlaces, min, max) {
  ...
  if (decimalPlaces != null) {
    value = numberFormat(value, decimalPlaces);
  }
...
function convertToDecimal(n, fromBase) {
  ...
  if (typeof fromBase != 'number' || fromBase != Math.round(fromBase) || fromBase < 2 || fromBase >
    36) {
    return undefined;
  }
  var result = 0;
